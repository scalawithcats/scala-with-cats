...
+ creating a base type for program algebras;
+ making the program type a type member;
+ defining a type for programs;
+ defining constructors on companion objects; and
+ using extension methods for combinators.
...
+ the program algebras are a single parameter to the method, which is possible because they extend a common base type;
+ the `Ui` type parameter is no longer needed, as it has become a type member; and
+ we must now use a dependent method to specify the result type.
...
It's still quite involved to write this code, though we can simplify it a bit by using the _single abstract method_ technique, which means a `trait` with a single abstract method (like `Program`) can be implemented with a function.
