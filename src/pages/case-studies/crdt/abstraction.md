...
Although the structure of the implementation will be the same for most of the type class instances we won't get any code reuse.

One solution is to capture the idea of a key-value store within a type class, and then generate `GCounter` instances for any type that has a `KeyValueStore` instance.
Here's the code for `KeyValueStore`, including syntax and an example instance for `Map`.
...
We could cut down on the boilerplate by using compiler plugins such as Simulacrum and kind-projector.
